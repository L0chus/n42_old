...
window.Vaadin.Flow['_vaadintheme_n42_globalCss'] = window.Vaadin.Flow['_vaadintheme_n42_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_n42_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_n42_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_n42_globalCss'].push(target);
  }
  if (!document['_vaadintheme_n42_componentCss']) {
    
    document['_vaadintheme_n42_componentCss'] = true;
  }
